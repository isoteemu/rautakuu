...
sub getServer
{
	...
	if ($result->rows)
	{
		my ($serverId, $name, $game) = $result->fetchrow_array;
		$result->finish;

		return new HLstats_Server($serverId, $address, $port, $name, $game);
	}
	else
	{
		$result->finish;
...
HLstats: http://www.unitedadmins.com/hlstats.php
EOT
;
...
;
	...
	if ($c % 500 == 0 && $g_deletedays)
	{
		...
		foreach $eventTable (keys(%g_eventTables))
		{
			if ($g_debug > 0)
			{
				print "-> hlstats_Events_$eventTable ... "
			}
