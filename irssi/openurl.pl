...
# 05.06.2002
# * complete rewrite
# 19.12.2005 Teemu A <teemu@terrasolid.fi>
# * Use SSH to remotely open urls
...
$VERSION = "20051219";
%IRSSI = (
    authors     => "Stefan 'tommie' Tomanek, Teemu A",
    contact     => "teemu\@terrasolid.fi",
    name        => "OpenURL",
...
Irssi::settings_add_bool('openurl', 'openurl_use_ssh', 1);
Irssi::settings_add_str('openurl', 'openurl_ssh_client', Irssi::settings_get_str('nick').'@localhost');

Irssi::settings_add_str('openurl', 'openurl_app_http', 'DISPLAY=:0 /usr/kde/3.5/bin/konqueror "$1"');
Irssi::settings_add_str('openurl', 'openurl_app_ftp', 'DISPLAY=:0 /usr/kde/3.5/bin/kfmclient exec "$1"');
Irssi::settings_add_str('openurl', 'openurl_app_mail', 'DISPLAY=:0 /usr/kde/3.5/bin/kmail --composer "$1"');
...
Settings you can change with /SET
    openurl_use_ssh:    Use SSH to execute command.
    openurl_ssh_client: IP or hostname of the remote pc.

    openurl_app_http:   Program to use opening webpages.
    openurl_app_ftp:    Program to use opening FTP connections.
    openurl_app_mail:   Mail composer program.
";
    my $text = '';
    foreach (split(/\n/, $help)) {
        $_ =~ s/^\/(.*)$/%9\/$1%9/;
...
sub launch_url ($) {
    ...
    my $use_ssh = Irssi::settings_get_bool('openurl_use_ssh');
    if($use_ssh == 1) {
        my $ssh_client = Irssi::settings_get_str("openurl_ssh_client");
        return system("ssh -f $ssh_client 'eval $command > /dev/null 2>&1'");
    } else {
